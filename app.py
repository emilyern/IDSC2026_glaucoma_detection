...
def make_gradcam_heatmap(img_array, model):
    import tensorflow as tf  # keras 3 still uses tf backend for GradientTape
    ...
    if base_model is None:
        raise ValueError("EfficientNetB0 not found.")
 
    _ = base_model(img_array, training=False)
 
    last_conv_layer = base_model.get_layer("top_conv")
 
    full_grad_model = keras.models.Model(
    ...
    with tf.GradientTape() as tape:
        conv_outputs, base_output = full_grad_model(img_tensor, training=False)
    ...
    if grads is None:
        raise ValueError("Gradients are None — tape lost connection.")
...
if st.checkbox("Show GradCAM heatmap (which region influenced the prediction)"):
    try:
        ...
        st.warning(f"GradCAM failed: {e}. Check that model.layers[0] is EfficientNetB0.")
